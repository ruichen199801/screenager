package com.ruichen.screenager.external;
...
public class TwitchClient {
    private static final String TOKEN = "Bearer placeholder"; // token-based auth, expire in around 2 months
    private static final String CLIENT_ID = "placeholder";
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) { // empty: return top 20 top games
            return String.format(url, limit);
        } else { // else: return a specific game url
            try {
                // Encode special characters in URL, e.g. Ruichen Z -> Ruichen20%Z
    ...
    // Send HTTP request to Twitch Backend based on the given URL (using Apache HTTP client library),
    ...
    // Return items specified by type
    ...
    // Return items not specified by type
